...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        if (req.method === 'GET') {
            ...
            const external = await externalApiServerFetch(`/factus/clientes${q.toString() ? `?${q}` : ''}`, { method: 'GET' }, token);
            return res.status(200).json({ success: true, data: external?.data ?? external });
        }
        if (req.method === 'POST') {
            const external = await externalApiServerFetch('/factus/clientes', { method: 'POST', body: JSON.stringify(req.body) }, token);
