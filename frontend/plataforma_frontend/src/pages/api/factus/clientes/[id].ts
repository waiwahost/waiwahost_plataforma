...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        if (req.method === 'GET') {
            const external = await externalApiServerFetch(`/factus/clientes/${id}`, { method: 'GET' }, token);
            return res.status(200).json({ success: true, data: external?.data ?? external });
        }
        if (req.method === 'PUT') {
            const external = await externalApiServerFetch(`/factus/clientes/${id}`, { method: 'PUT', body: JSON.stringify(req.body) }, token);
            return res.status(200).json({ success: true, data: external?.data ?? external });
        }
        if (req.method === 'DELETE') {
            await externalApiServerFetch(`/factus/clientes/${id}`, { method: 'DELETE' }, token);
