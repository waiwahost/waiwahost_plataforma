...
// Handles:
//   GET  /api/factus/documentos-soporte/[id]         → detail
//   POST /api/factus/documentos-soporte/[id]/enviar  → enviar a DIAN
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        if (isEnviar && req.method === 'POST') {
            const ext = await externalApiServerFetch(`/factus/documentos-soporte/${id}/enviar`, { method: 'POST', body: '{}' }, token);
            return res.status(200).json({ success: true, data: ext?.data ?? ext });
        }
        if (req.method === 'GET') {
            const ext = await externalApiServerFetch(`/factus/documentos-soporte/${id}`, { method: 'GET' }, token);
            return res.status(200).json({ success: true, data: ext?.data ?? ext });
        }
