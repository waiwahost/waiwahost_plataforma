...
const IncomesTable: React.FC<IncomesTableProps> = ({
  ...
  const getTipoIngresoText = (tipo: 'movimiento' | 'pago') => {
    return tipo === 'movimiento' ? 'Movimiento' : 'Pago';
  };
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-800">Lista de Ingresos</h2>
        </div>
        <div className="p-4">
            ...
            {ingresos.map((ingreso) => (
              <tr key={ingreso.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  {formatTime(ingreso.fecha_creacion)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm font-medium text-gray-900">
                    {getConceptoText(ingreso.concepto)}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="text-sm text-gray-900 max-w-xs truncate" title={ingreso.descripcion}>
                    {ingreso.descripcion}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">
                    {ingreso.nombre_inmueble}
                  </div>
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-900">
                      {getMetodoPagoText(ingreso.metodo_pago)}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTipoIngresoColor(ingreso.tipo_ingreso)}`}>
