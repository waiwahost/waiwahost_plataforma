...
export class GetInmueblesService {
  ...
  async execute(
    ...
    try {
      ...
      if (idInmueble) {
        const { data: inmueble, error } = await this.inmueblesRepository.getInmuebleById(idInmueble);
        ...
        if (userContext.id_roles === ROLES.EMPRESA || userContext.id_roles === ROLES.ADMINISTRADOR) {
          ...
          if (Number(inmueble.id_empresa) !== Number(userContext.empresaId)) {
        ...
        if (userContext.id_roles === ROLES.PROPIETARIO) {
          ...
          const { data: propietarioId, error: propError } = await this.inmueblesRepository.getPropietarioIdByUserId(userContext.id);
          ...
          if (Number(inmueble.id_empresa) !== Number(userContext.empresaId) || Number(inmueble.id_propietario) !== Number(propietarioId)) {
            return { data: null, error: { status: 403, message: 'No tiene permisos para ver este inmueble', details: null } };
          }
          return { data: inmueble, error: null };
        }
      ...
      if (userContext.id_roles === ROLES.SUPERADMIN) {
        if (idEmpresa) {
          ...
          const { data: inmuebles, error } = await this.inmueblesRepository.getInmueblesByEmpresa(idEmpresa);
          if (error) {
          ...
          const { data: inmuebles, error } = await this.inmueblesRepository.getAllInmuebles();
          if (error) {
            return { data: null, error: { status: 500, message: 'Error al obtener los inmuebles', details: error } };
          }
          return { data: inmuebles, error: null };
      ...
      if (userContext.id_roles === ROLES.EMPRESA || userContext.id_roles === ROLES.ADMINISTRADOR) {
        ...
        const { data: inmuebles, error } = await this.inmueblesRepository.getInmueblesByEmpresa(userContext.empresaId);
        if (error) {
          return { data: null, error: { status: 500, message: 'Error al obtener los inmuebles', details: error } };
        }
        return { data: inmuebles, error: null };
      ...
      if (userContext.id_roles === ROLES.PROPIETARIO) {
        ...
        const { data: inmuebles, error } = await this.inmueblesRepository.getInmueblesByEmpresaAndPropietario(
...
export const getInmueblesService = new GetInmueblesService();